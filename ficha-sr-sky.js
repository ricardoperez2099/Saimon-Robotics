(function () {
  ...
  const specSets = {
    autonomo: [
      { label: 'Vuelo', value: 'Autónomo', note: 'Rutas programadas sin necesidad de piloto constante.' },
      { label: 'Autonomía', value: '40 min', note: 'Baterías intercambiables para cobertura prolongada.' },
      { label: 'Navegación', value: 'GPS + rutas', note: 'Trayectorias predefinidas por zona o perímetro.' }
    ],
    vigilancia: [
      { label: 'Transmisión', value: 'Tiempo real', note: 'Video en vivo hacia el centro de monitoreo.' },
      { label: 'Cámara', value: 'Gran angular + zoom', note: 'Captura detallada desde cualquier altitud.' },
      { label: 'Integración', value: 'SaimonIQ', note: 'Ubicación, batería y video centralizados.' }
    ],
    obstaculos: [
      { label: 'Detección', value: 'Tiempo real', note: 'Evasión de obstáculos durante el vuelo.' },
      { label: 'Sensores', value: 'Multidireccionales', note: 'Cobertura alrededor de toda la plataforma.' },
      { label: 'Seguridad', value: 'Vuelo estable', note: 'Ajuste automático de ruta ante obstáculos.' }
    ],
    recarga: [
      { label: 'Recarga', value: 'Automática', note: 'Retorno a base al alcanzar batería mínima.' },
      { label: 'Baterías', value: 'Intercambiables', note: 'Cobertura prolongada por relevo de baterías.' },
      { label: 'Monitoreo', value: 'SaimonIQ', note: 'Estado de batería visible en tiempo real.' }
    ]
  };

  const opsPosters = {
    autonomo: { poster: IMG + 'srsky-city-flight.png', alt: 'Vuelo autónomo sobre entorno urbano' },
    vigilancia: { poster: IMG + 'srsky-camera-detail.png', alt: 'Vigilancia en tiempo real' },
    obstaculos: { poster: IMG + 'srsky-fence-flight.png', alt: 'Detección de obstáculos' },
    recarga: { poster: IMG + 'srsky-urban-flight.png', alt: 'Recarga automática' }
  };
  ...
  function renderSpecs(key) {
    const rows = specSets[key] || specSets.autonomo;
  ...
  function loadOpsPoster(key) {
    const clip = opsPosters[key] || opsPosters.autonomo;
